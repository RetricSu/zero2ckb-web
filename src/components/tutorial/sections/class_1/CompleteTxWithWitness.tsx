...
export default function CompleteTxWithWitness(
  props: CompleteTxWithWitnessProps
) {
  ...
  const onCompleteTx = () => {
    var rtx = eval(
      "`" +
        JSON.stringify(raw_tx).substring(1, JSON.stringify(raw_tx).length - 1) +
        "`"
    );
    ...
    if (onCallBack && myTx) onCallBack(myTx);
  };
  ...
  return (
    <div style={styles.root}>
      ...
      <FreshButton
        ...
        text={"witnesses 加入 raw_tx"}
        custom_style={{ width: "100%", fontSize: "16px" }}
      ></FreshButton>
      <div style={styles.result}>
        <CodePiece
          code={tx || ""}
          custom_style={{ border: "0", padding: "10px" }}
        />
      </div>
    </div>
  );
