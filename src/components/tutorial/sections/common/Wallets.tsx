...
export function WalletInfo(props: WalletInfoProps) {
  ...
  const info = () => {
    if (isShowing) {
        ...
        <div style={styles.wallet_info}>
          ...
          <div style={styles.wallet_info_text}>
            <p>
              ...
              {wallet.mainnet.slice(0, 8)}..
              {wallet.mainnet.slice(wallet.mainnet.length - 5)}{" "}
              <CopyText text={wallet.mainnet} icon={true} />
            </p>
            <p>
              <strong style={styles.main_color}>
                testnet:
              ...
              {wallet.testnet.slice(0, 8)}..
              {wallet.testnet.slice(wallet.testnet.length - 5)}{" "}
              <CopyText text={wallet.testnet} icon={true} />
            </p>
            <p>
              <strong style={styles.main_color}>
                lock_arg: &#160;&#160;&#160;&#160;&#160;
              </strong>
              {wallet.lock_arg.slice(0, 8)}..
              {wallet.lock_arg.slice(wallet.lock_arg.length - 5)}{" "}
              <CopyText text={wallet.lock_arg} icon={true} />
            </p>
            <p>
              <strong style={styles.main_color}>private_key: </strong>
              {wallet.private_key.slice(0, 8)}..
              {wallet.private_key.slice(wallet.private_key.length - 5)}{" "}
              <CopyText text={wallet.private_key} icon={true} />
            </p>
          </div>
          <CodePiece
            custom_style={{
      ...
      return (
        <CodePiece
          custom_style={{ border: "0px", marginTop: "0px" }}
          code={wallet_arcii_2}
        ></CodePiece>
      );
    }
...
export default function Wallets(props: Props) {
  ...
  async function fetchWallets() {
      ...
      myWallets.map((wallet: Wallet, index: number) => {
        return (
          <li key={index} style={styles.wallet_panel}>
            <Typography style={styles.main_color}>
              {" "}
              {t("tutorial.widget.wallets.wallet")} {index + 1}{" "}
            </Typography>
    ...
    if (props.onFetchWallets) props.onFetchWallets(myWallets);
  }
    ...
    <Container
      style={
        custom_style != undefined
          ? { ...styles.wallet_section, ...custom_style }
          : styles.wallet_section
      }
    >
      ...
      <div style={styles.wallets}>
        {props.wallet_id ? wallets[props.wallet_id - 1] : wallets}
      </div>
      <p style={{ clear: "both" }} />
      <br />
    </Container>
