      ...
      <div style={{ ...styles.edit_cell, display }}>
        ...
        <span style={styles.input_wrap}>
          <input
            ...
            style={styles.input}
            type="number"
            placeholder={t("tutorial.common.decimalInputPlaceHolder")}
          />
        </span>
        ...
        <span style={{ ...styles.input_wrap, ...hideDisableInput }}>
          <input
            style={styles.input}
            value={"code_hash: " + config?.SCRIPTS.SECP256K1_BLAKE160.CODE_HASH}
            disabled
          />
        </span>
        <span style={styles.input_wrap}>
          <input
            ...
            style={styles.input}
            type="text"
            placeholder={t("tutorial.common.hexInputPlaceHolder")}
          />
        </span>
        <span style={{ ...styles.input_wrap, ...hideDisableInput }}>
          <input
            style={styles.input}
            value={"hash_type: " + config?.SCRIPTS.SECP256K1_BLAKE160.HASH_TYPE}
            disabled
          />
        </span>
        ...
        <span style={{ ...styles.input_wrap, ...hideDisableInput }}>
          <input
            style={styles.input}
            value={config?.SCRIPTS.SECP256K1_BLAKE160.CODE_HASH}
            disabled
          />
        </span>

        <p>
          <button style={styles.edit_cell_save_btn} onClick={save_cell}>
            {t("tutorial.common.saveBtnText")}
          </button>
        </p>
      </div>
      <div
        style={{
          ...styles.final_cell,
          ...{ display: isFinalCellOpen ? "inline-block" : "none" },
        }}
      >
...
export default function EditOutputCells(props: EditOutputCellsProps) {
  ...
  const apply = () => {
          ...
          return {
            cell_output: {
              ...
              lock: {
                ...
                hash_type:
                  config?.SCRIPTS.SECP256K1_BLAKE160.HASH_TYPE || "type",
                args: cell.lock_args,
              },
            },
            data: cell.data,
    ...
    if (trigger_modal_close) trigger_modal_close();
  };
    ...
    <div style={styles.root}>
      <div style={styles.edit_cell_header}>
        ...
        <span style={styles.right_btn}>
          <button onClick={add_cell}>
            {" "}
            {t("tutorial.widget.editOutputCells.addNewCellBtnText")}{" "}
          </button>{" "}
          <button onClick={apply}>
            {" "}
            {t("tutorial.widget.editOutputCells.saveExitBtnText")}{" "}
          </button>
        </span>
      </div>
      ...
      <div style={styles.op_panel}>
        {new_cells.map((cell: SimpleCellJson) => {
          return (
            <PlainCell
              t={t}
              total_capacity={utils.shannon2CKB(utils.hex2dec(capacity))}
              cell={cell}
              config={config}
              get_final_cell={onSaveCell}
            />
          );
        })}
      </div>
