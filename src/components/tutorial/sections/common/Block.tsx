...
export default function BlockBox(props: BlockProps) {
  ...
  function show_tx_list(transactions: Transaction[]) {
    const jsx = transactions.map((tx: Transaction, index: number) => (
      <ShowTxInfo tx={tx} key_id={index} />
    ));
    return jsx;
  }
  ...
  return (
    <div>
      <Container
        style={custom_style ? { ...box_style, ...custom_style } : box_style}
        key={block.header.hash}
        onMouseEnter={hovering}
        onMouseLeave={unHover}
      >
        <div style={styles.box_header}>
          <div style={styles.box_header_title}>
            {" "}
            {t("tutorial.widget.block.blockText")} #
            {Utils.hex2dec(block.header.number)}{" "}
          </div>
          <p style={styles.box_header_sub_title}>
            {t("tutorial.widget.block.hashText")}{" "}
            {block.header.hash.slice(0, 12)}..{" "}
          </p>
          <p style={styles.box_header_sub_title}>
            {t("tutorial.widget.block.timeText")}{" "}
            {Utils.convertTimestamp(
              BigInt(block.header.timestamp).toString(10)
            )}{" "}
          </p>
        </div>
        <hr />
        <p style={styles.box_header_sub_title}>
          {t("tutorial.widget.block.transactionCount", {
            count: block.transactions.length,
          })}
        </p>
        <div style={styles.box_content}>
          <ul style={styles.ul}>{show_tx_list(block.transactions)}</ul>
        </div>
      </Container>
    </div>
  );
