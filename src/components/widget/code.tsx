...
const CodePeice = forwardRef((prop: CodeProp, ref: Ref<CodePieceType>) => {
  ...
  return (
    <div>
      <pre
        ref={preRef}
        contentEditable={prop.isContentEditable ? "true" : "false"}
        onInput={getContent}
        style={styles.code_box}
        onPaste={(e) => onPasteClean(e)}
      >
        {typeof prop.code === "string" && prop.code}
        {typeof prop.code === "object" && JSON.stringify(prop.code, null, 2)}
      </pre>
    </div>
  );
});
